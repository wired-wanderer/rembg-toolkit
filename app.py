...
class BatchProcessThread(QThread):
    ...
    def __init__(self, file_paths, output_dir, options, model_name, use_custom_onnx,
        ...
        self.use_custom_onnx = use_custom_onnx        # ← 追加
        self.custom_onnx_path = custom_onnx_path
        self.mask_blur = mask_blur
        self.mask_threshold = mask_threshold
        self.spill_enabled = spill_enabled            # ← 追加
        self.spill_threshold = spill_threshold         # ← 追加
        self.spill_patch = spill_patch                 # ← 追加
    ...
    def _estimate_background_color(img: Image.Image, patch: int = 10) -> np.ndarray:
        ...
        h, w = arr.shape[:2]
        patch = max(1, min(patch, h // 2, w // 2))  # 画像サイズより大きくならないよう安全策
        corners = [
            arr[0:patch, 0:patch],
    ...
    def run(self):
        ...
        for index, file_path in enumerate(self.file_paths):
            ...
            try:
                ...
                # --- 変更: スピル除去をチェックボックスで制御 ---
                if self.spill_enabled:
                    orig_for_spill = Image.open(io.BytesIO(input_bytes)).convert("RGB")
                    alpha = self._suppress_color_spill(
                        orig_for_spill, alpha,
                        threshold=self.spill_threshold, patch=self.spill_patch
                    )
...
class RembgGuiApp(QMainWindow):
    def __init__(self):
        ...
        # --- 追加: 自前ONNX格納フォルダ / プリセット保存フォルダ ---
        self.onnx_dir = os.path.join(base_dir, "onnx")
    ...
    def init_ui(self):
        ...
        left_layout.addWidget(self.file_list, stretch=1)  # ← stretch=1 に変更
        ...
        left_layout.addWidget(self.log_view, stretch=1)  # file_listと同じstretchで概ね半分ずつ
        ...
        output_form.setContentsMargins(5, 5, 5, 5)

        self.edit_output_dir = QLineEdit()
        self.edit_output_dir.setPlaceholderText("未設定（実行前に選択してください）")
        self.btn_browse_output = QPushButton("選択...")
        ...
        output_form.addRow(QLabel("保存先:"), output_dir_layout)
        ...
        self.btn_refresh_onnx.setToolTip(f"{self.onnx_dir} 内の.onnxファイルを再スキャンします")
        ...
        self.btn_browse_onnx.setToolTip(f"選択したファイルを {self.onnx_dir} にコピーして一覧に追加します")
        ...
        # --- 5. 画像プレビュー表示エリア ---
        preview_group = QGroupBox("選択中の画像プレビュー")
        preview_layout = QVBoxLayout()
        preview_layout.setContentsMargins(5, 5, 5, 5)
        
        self.preview_label = QLabel("リストから画像を選択すると表示されます")
        self.preview_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.preview_label.setStyleSheet("border: 1px dashed #cccccc; background-color: #fafafa; color: #888888;")
        ...
        self.refresh_onnx_list()       # ← 追加
        self.on_custom_toggle(None)    # ← 追加：初期状態のenabled/disabledを反映
        self.refresh_preset_list()  # ← 追加
    ...
    def refresh_onnx_list(self):
        ...
        files = sorted(f for f in os.listdir(self.onnx_dir) if f.lower().endswith(".onnx"))
        if not files:
    ...
    def browse_onnx_file(self):
        ...
        dest_path = os.path.join(self.onnx_dir, os.path.basename(file_path))
        try:
    ...
    def browse_output_dir(self):
        current = self.edit_output_dir.text().strip() or getattr(self, "last_output_dir", "")
    ...
    def refresh_preset_list(self):
        ...
        names = sorted(
            os.path.splitext(f)[0] for f in os.listdir(self.presets_dir) if f.lower().endswith(".json")
        )
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, "r", encoding="utf-8") as f:
                    ...
                    custom_path = config.get("custom_onnx_path", "")
                    if os.path.exists(custom_path):
                        self.edit_custom_path.setText(custom_path)

                    # --- 変更: 出力先フォルダをUIフィールドに直接復元 ---
                    self.last_output_dir = config.get("output_dir", "")
                    if os.path.exists(self.last_output_dir):
                        self.edit_output_dir.setText(self.last_output_dir)
                    else:
    ...
    def save_config(self):
        try:
            ...
            config["custom_onnx_path"] = self.edit_custom_path.text().strip()
            config["output_dir"] = self.edit_output_dir.text().strip()  # ← 変更: フィールドの値を直接保存
    ...
    # 自前ONNXファイルを選択するダイアログ
    def browse_onnx_file(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "自前ONNXモデルを選択", "", "ONNX Models (*.onnx)"
        )
        if file_path:
            self.edit_custom_path.setText(file_path)
            self.save_config()  # ← パスが選択されたら即座に外部記録（JSON保存）

    # 選択された画像のリアルタイムプレビュー表示
    def update_image_preview(self, current_item, previous_item):
        if not current_item:
            self.preview_label.setText("リストから画像を選択すると表示されます")
        ...
        if os.path.exists(file_path):
            ...
            if not pixmap.isNull():
                # プレビュー枠のサイズに合わせてアスペクト比を維持しつつ滑らかに縮小
            ...
            self.preview_label.setText("ファイルが見つかりません")
    ...
    def update_model_description(self, text):
        descriptions = {
            ...
            "bria-rmbg": "BRIA AIによる、商業利用でも最先端の背景除去モデル"
        }
    ...
    def start_batch_processing(self):
        ...
        if use_custom_onnx:
            ...
            if not selected_file or not selected_file.lower().endswith(".onnx"):
                ...
                return
            custom_onnx_path = os.path.join(self.onnx_dir, selected_file)
    ...
    def on_processing_finished(self, success_count):
        ...
        self.btn_cancel.setEnabled(False)  # ← 追加
        self.append_log(f"=== 完了: 処理に成功したファイル数 {success_count} 個 ===")
        # QMessageBox.information(...) は削除（ログで確認できるため）
